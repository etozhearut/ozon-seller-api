...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/info" \
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/list" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
Метод для получения отчёта с данными о товарах. Например, Ozon ID, количества товаров, цен, статуса. Соответствует разделу/действию Товары и цены → Список товаров → Скачать → Товары CSV в личном кабинете. Пояснения к некоторым полям: Ozon Product ID — идентификатор товара в нашей системе. …

```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/products/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v2/report/returns/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/postings/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/finance/cash-flow-statement/list" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
Запускает генерацию отчёта по уценённым товарам на складе Ozon. Ozon может сам уценить товар, например, при повреждении. В результате запроса будет не сам отчёт, а его уникальный идентификатор. Чтобы получить отчёт, отправьте идентификатор в запросе метода /v1/report/info . …

```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/discounted/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>"
```
...
Отчёт с информацией о количестве доступных и зарезервированных единиц товара на складе. Соответствует разделу FBS → Управление логистикой → Управление остатками → Скачать в XLS в личном кабинете. В результате запроса будет не сам отчёт, а его уникальный идентификатор. …

```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/warehouse/stock" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/placement/by-products/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/placement/by-supplies/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api-seller.ozon.ru/v1/report/marked-products-sales/create" \
  -H "Client-Id: <CLIENT_ID>" \
  -H "Api-Key: <API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
