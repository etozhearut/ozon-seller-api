...
Используйте метод, чтобы получить аналитику по остаткам товаров на складах. Метод соответствует разделу FBO → Управление остатками в личном кабинете. Аналитика обновляется два раза в день: примерно в 07:00 и 16:00 по UTC. В запросе используйте только одно из полей: cluster_ids или macrolocal_cluster_ids , иначе вернётся ошибка.
